...
def gen_img_yor_text(phrase_yoruba):
  ...
  # Convertir l'image PIL en données binaires
...
def translate_text(language):
    ...
    if language == lg_fon: 
        translated_text = translate_yor_en(input_text)
        text_result.value = f"Traduction Anglais: {translated_text}"
    elif language == lg_yor:
        translated_text = translate_yor_en(input_text)
        text_result.value = f"Traduction Français: {translated_text}"
...
yoruba_to_french_button.on_click(lambda button: translate_text(lg_yor))
fon_to_french_button.on_click(lambda button: translate_text(lg_fon))
yoruba_image_button.on_click(lambda button: generate_image(lg_yor))
fon_image_button.on_click(lambda button: generate_image(lg_fon))
